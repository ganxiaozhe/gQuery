...
__page.init.data = function(){
    ...
    $.fetch(`https://acp.53iq.com/acp/?id=${pata.id}&token=${pata.token}`, 'json').then(rsp=>{
        if(rsp.code!==0){
...
__page.do.gotData = function(){
    ...
    let __json = {title:{
        background: {url: __cd.cookbook_image},
        text: {
            headline: `<div style='font-size:0.5em;opacity:0.7'>制作:</div> ${__cd.name}`
        }
    }, events:[]};
    ...
    __tips.map((li, idx)=>{
        __json.events.push({
            media: {url:li.path},
            start_date: {
                year: __year, second: li.time*60,
                display_date: `${li.time} 分钟`
            },
            text: {
                "headline": li.content,
                "text": li.desc||'暂无描述'
    ...
    pata.json = __json;
    pata.images = __tips.map(li=>li.path);

    __page.do.timeline();
    $('body').loader(false);

    // 菜谱标题
    $('#cookCard-title').text(__cd.name);
    $('#app_title').text(`菜谱: ${__cd.name}`);
    pata.shareInfo = {
        title:'菜谱: '+__cd.name,
        desc:'快来看看这个菜谱！拜托，超级美味的！',
        link:window.location.href.replace(/#.*/g, ''), // 分享链接，该链接域名或路径必须与当前页面对应的公众号 JS 安全域名一致
        imgUrl:__cd.cookbook_image
    }
...
__page.do.timelineDone = function(){
    ...
    $('.tl-slide.tl-slide-titleslide').append(`<div id='user-thumb' class='user-thumb'>
        ...
        <div class='user-thumb-item'>
            ...
            <div class='user-thumb__text'>${__cd.collect_num}</div>
        </div>
